    ...
    , ui(new Ui::MainWindow)
{
    ui->setupUi(this);
    // --------------------------- basic backgroud --------------------
    image = new QImage(30*23,30*23,QImage::Format_RGB32);
...
{
    paintOnImage->fillRect(QRect(0,0,30*23,30*23),QColor(180,180,170));
    ...
    for (unsigned short int y=0; y<30; y++)
    {
        for (unsigned short int x=0; x<30; x++)
        {
            switch (myBoard.get(y,x)) {
                ...
                {
                    paintOnImage->drawImage(QRectF(x*23,y*23,23,23),QImage(":/img/wall.png"),source);
                    break;
                }
                case OneCell::CELL_DOOR:
                {
                    paintOnImage->drawImage(QRectF(x*23,y*23,23,23),QImage(":/img/door.png"),source);
                    break;
                }
                case OneCell::CELL_STEVE:
                {
                    paintOnImage->drawImage(QRectF(x*23,y*23,23,23),QImage(":/img/steve.png"),source);
                    break;
                }
                case OneCell::CELL_HOLE:
                {
                    paintOnImage->drawImage(QRectF(x*23,y*23,23,23),QImage(":/img/hole.png"),source);
                    break;
                }
                case OneCell::CELL_DIAMOND:
                {
                    paintOnImage->drawImage(QRectF(x*23,y*23,23,23),QImage(":/img/diamond.png"),source);
                    break;
                }
                case OneCell::CELL_DIAMONDinHOLE:
                {
                    paintOnImage->drawImage(QRectF(x*23,y*23,23,23),QImage(":/img/diamondHole.png"),source);
                    break;
                }
                case OneCell::CELL_STEVEinHOLE:
                {
                    paintOnImage->drawImage(QRectF(x*23,y*23,23,23),QImage(":/img/steveHole.png"),source);
                    break;
                }
                case OneCell::CELL_ARROW_UP:
                {
                    paintOnImage->drawImage(QRectF(x*23,y*23,23,23),QImage(":/img/arrowUp.png"),source);
                    break;
                }
                case OneCell::CELL_ARROW_DOWN:
                {
                    paintOnImage->drawImage(QRectF(x*23,y*23,23,23),QImage(":/img/arrowDown.png"),source);
                    break;
                }
                case OneCell::CELL_ARROW_LEFT:
                {
                    paintOnImage->drawImage(QRectF(x*23,y*23,23,23),QImage(":/img/arrowLeft.png"),source);
                    break;
                }
                case OneCell::CELL_ARROW_RIGHT:
                {
                    paintOnImage->drawImage(QRectF(x*23,y*23,23,23),QImage(":/img/arrorRight.png"),source);
                    break;
                }
            }
        }
...
void MainWindow::clickRIGHT()
{
    ...
    //steve and diamodn and empty
    else if ((myBoard.get(steveY,steveX+1)==OneCell::CELL_DIAMOND || myBoard.get(steveY,steveX+1)==OneCell::CELL_DIAMONDinHOLE))
    {
        int flag = 1;
        int count = 0;
        int door= 0;
        while (myBoard.get(steveY,steveX+count)!=OneCell::CELL_EMPTY)
        {
            if (myBoard.get(steveY,steveX+count)==OneCell::CELL_WALL)
            {
                flag = 0;
                break;
            }
        ...
        if (flag)
        {
            ...
            {
                int tempx = steveX+count-i-1;
                int tempy = steveY;
                int temp = myBoard.get(tempy,tempx);
                //向右一格
                myBoard.set(tempy,tempx+1,temp);
                //原来的位置设置为空    
                myBoard.set(tempy,tempx,OneCell::CELL_EMPTY);
            }
            myBoard.set(steveY,steveX,(myBoard.get(steveY,steveX)==OneCell::CELL_STEVE)?OneCell::CELL_EMPTY:OneCell::CELL_HOLE);
            myBoard.set(steveY,steveX+1,OneCell::CELL_STEVE);
        }
