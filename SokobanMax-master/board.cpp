...
void Board::load(QVector<unsigned short int> level)
{
    ...
    for (unsigned short int y=0; y<30; y++)
    {
        for (unsigned short int x=0; x<30; x++)
        {
            myBoard[y][x].set(level[30*y+x]);
            if (level[30*y+x]==OneCell::CELL_STEVE || level[30*y+x]==OneCell::CELL_STEVEinHOLE)
            {
                pos_Steve_x=x;
                pos_Steve_y=y;
            }
            if (level[30*y+x]==OneCell::CELL_ARROW_DOWN)
            {
                pos_Teleport_x=x;
                pos_Teleport_y=y+1;
            ...
            if (level[30*y+x]==OneCell::CELL_ARROW_UP)
            {
                pos_Teleport_x=x;
                pos_Teleport_y=y-1;
                pos_Teleport_x_next=x;
            ...
            if (level[30*y+x]==OneCell::CELL_ARROW_RIGHT)
            {
                pos_Teleport_x=x+1;
                pos_Teleport_y=y;
            ...
            if (level[30*y+x]==OneCell::CELL_ARROW_LEFT)
            {
                pos_Teleport_x=x-1;
                pos_Teleport_y=y;
                pos_Teleport_x_next=x-2;
...
{
    if (y>49 || y<0 || x<0 || x>49) return OneCell::CELL_WALL;
